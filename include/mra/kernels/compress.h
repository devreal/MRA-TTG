...
namespace mra {
  ...
  namespace detail {
     ...
     * block id). tmp is sized for only the non-zero functions, so it is
     * indexed by the compact launch position `pos` (0..n_nonzero-1); d_sumsq
     * stays sized for all N (read back by real function id host-side after
     * the kernel returns) and is indexed by the real, remapped `fnid`.
     */
    template<typename T, Dimension NDIM>
      ...
      size_type fnid)
    {
      ...
      assert(!(result_in.is_zero(fnid) && p_in.is_zero(fnid)) &&
             "fnid must have result or p non-zero -- caller derives it via find_nth_nonzero");
      if (is_team_lead()) {
        T* block_tmp = &tmp[pos*compress_tmp_size<NDIM>(K)];
      ...
      const concepts::TensorViewArray<NDIM+1, Key<NDIM>::num_children()> auto in_views)
    {
      for (size_type pos = blockIdx.x; pos < n_nonzero; pos += gridDim.x) {
        size_type fnid = find_nth_nonzero(N, pos, result_in, p_in);
        compress_process_one<T, NDIM>(key, K, is_ns, node_in, p_in, result_in, hgT,
                                      tmp, d_sumsq, in_views, pos, fnid);
      }
    }
  } // namespace detail
  ...
  namespace detail {
    ...
    using CompressBatchArg = std::tuple<
      ...
      size_type                                                          // n_nonzero: count of functions with p or result non-zero (tmp is sized for this, not n)
    >;
    ...
    struct CompressBatchArgIdx {
      ...
      static constexpr std::size_t n_nonzero       = 9;
    };
    ...
    GLOBALSCOPE void compress_kernel_batched(
      ...
      const concepts::TensorView<2> auto hgT)
    {
      using argidx = CompressBatchArgIdx;
      ...
      auto& p_in = std::get<argidx::p_in>(arg);
      auto& result_in = std::get<argidx::result_in>(arg);

      for (size_type pos = blockIdx.x; pos < n_nonzero; pos += gridDim.x) {
        size_type fnid = find_nth_nonzero(n, pos, result_in, p_in);
        compress_process_one<T, NDIM>(std::get<argidx::key>(arg), K, is_ns,
                                      std::get<argidx::node_in>(arg), p_in,
                                      result_in, hgT,
                                      std::get<argidx::tmp>(arg), std::get<argidx::d_sumsq>(arg),
                                      std::get<argidx::in_views>(arg), pos, fnid);
      }
    }
  ...
  void submit_compress_kernel_batched(
    ...
    ttg::device::Stream stream)
  {
    using argidx = detail::CompressBatchArgIdx;
  ...
  namespace detail {
      ...
      const concepts::TensorView<2> auto& hgT)
    {
      ...
      // First pass: total sparsity bytes needed across the whole batch (2*n
      // per member: p's bytes then result's bytes), so the pinned staging
      // buffer below is acquired/resized exactly once.
      ...
      for (std::size_t m = 0; m < nb; ++m) {
        ...
        const size_type n = static_cast<size_type>(m_result_in.dim(0));
        const size_type n_nonzero = count_nonzero_any(n, m_p_tensor, m_result_tensor);
        ...
        slot.host_args.emplace_back(m_key, m_node_in, m_p_in, m_result_in,
                                    ...
                                    m_in_views, n, sparsity_offset, n_nonzero);
        sparsity_offset += 2 * n;
      }

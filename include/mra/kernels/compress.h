...
namespace mra {
  ...
  void submit_compress_kernel_batched(
    detail::GroupedBatchPool<detail::CompressBatchArg<T, NDIM>>& pool,
    typename detail::GroupedBatchPool<detail::CompressBatchArg<T, NDIM>>::slot_t& slot,
    size_type total_nonzero,
    size_type K,
    bool is_ns,
    const HgtT& hgT,
    ttg::device::Stream stream)
  {
    ...
    CALL_KERNEL((detail::compress_kernel_batched<T, NDIM>), total_nonzero, thread_dims, smem_size, stream,
                (slot.dev_args, slot.dev_offsets, num_members, total_nonzero, K, is_ns, hgT));
    checkSubmit();
  ...
  namespace detail {
    ...
    void submit_compress_batch_leader(
      BatchView& batch,
      GroupedBatchPoolRegistry<CompressBatchArg<T, NDIM>>& registry,
      size_type K,
      bool is_ns,
      const HgtT& hgT,
      size_type total_functions)
    {
      ...
      submit_compress_kernel_batched<T, NDIM>(pool, slot, total_nonzero, K, is_ns, hgT, ttg::device::current_stream());
    }

...
namespace mra {
  ...
  SCOPE void parent_to_child(
    ...
    const concepts::tensor_view_nd<NDIM> auto& coeffs,
    concepts::tensor_view_nd<NDIM> auto& result,
    concepts::tensor_view_nd<NDIM> auto& result_tmp,
    const concepts::tensor_view_nd<2> auto& phibar,
    const concepts::tensor_view_nd<2> auto& phi,
    const concepts::tensor_view_nd<1> auto& quad_x,
    const size_type K,
    T* tmp)
    {
  ...
  namespace detail {
    ...
    DEVSCOPE void derivative_inner(
      ...
      const concepts::tensor_view_nd<NDIM> auto& node_left,
      const concepts::tensor_view_nd<NDIM> auto& node_center,
      const concepts::tensor_view_nd<NDIM> auto& node_right,
      const concepts::tensor_view_3d auto& operators,
      concepts::tensor_view_nd<NDIM> auto& deriv,
      concepts::tensor_view_nd<NDIM+1> auto& tmp,
      concepts::tensor_view_nd<NDIM> auto& left_tmp,
      concepts::tensor_view_nd<NDIM> auto& center_tmp,
      concepts::tensor_view_nd<NDIM> auto& right_tmp,
      const concepts::tensor_view_2d auto& phi,
      const concepts::tensor_view_2d auto& phibar,
      const concepts::tensor_view_1d auto& quad_x,
      const int bc_left,
      const int bc_right,
      size_type axis,
      size_type K,
    ...
    GLOBALSCOPE void derivative_kernel(
      ...
      const Key<NDIM> center,
      const Key<NDIM> right,
      const concepts::tensor_view_nd<NDIM+1> auto node_left,
      const concepts::tensor_view_nd<NDIM+1> auto node_center,
      const concepts::tensor_view_nd<NDIM+1> auto node_right,
      const concepts::tensor_view_3d auto operators,
      concepts::tensor_view_nd<NDIM+1> auto deriv,
      const concepts::tensor_view_2d auto phi,
      const concepts::tensor_view_2d auto phibar,
      const concepts::tensor_view_1d auto quad_x,
      T* tmp,

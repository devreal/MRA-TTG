...
namespace mra{
  ...
  auto make_norm(const std::shared_ptr<FunctionSetT>& fns,
    ...
    auto norm_fn = [fns, K, name](const mra::Key<NDIM>& key,
      ...
      /* only functions where in is non-zero get a thread-block launched, so
       * pre-fill with 0 for the rest -- matches the norm the kernel used to
       * write for them directly (this buffer is read back by real function id
       * by callers upstream). */
      std::fill(norms_result.buffer().host_ptr(), norms_result.buffer().host_ptr() + N, T(0));
      const size_type n_nonzero = count_nonzero_any(N, in.coeffs());

...
void test_coeffs(int argc, char** argv) {
  ...
  std::shared_ptr<const mra::OperatorData<double, 3>> op_data = op.get_op(mra::Key<3>(3, {1, 1, 1}));
  ...
  madness::Tensor<double> rnlij1_mad = conv1d.rnlij(2, 1);
  const madness::Tensor<double>& rnlp2_mad = conv1d.rnlp(2, -1);
  madness::Tensor<double> rnlij2_mad = conv1d.rnlij(2, -1);
  const madness::ConvolutionData1D<double>* cd_mad = conv1d.nonstandard(3, 1);
